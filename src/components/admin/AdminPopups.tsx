...
const AdminPopups = () => {
  ...
  const [spin, setSpin] = useState({
    enabled: true,
    title: 'UP TO 70% OFF',
    subtitle: 'Try your luck — spin the wheel!',
    segments: DEFAULT_SPIN_SEGMENTS as SpinSegment[],
    delay: '1', autoClose: '5', startAt: '', endAt: '',
  });
  const [spinInit, setSpinInit] = useState(false);
  useEffect(() => {
    if (!storeSettings || spinInit) return;
    let segs: SpinSegment[] = DEFAULT_SPIN_SEGMENTS;
    try {
      if (storeSettings.spin_wheel_segments) {
        const parsed = JSON.parse(storeSettings.spin_wheel_segments);
        if (Array.isArray(parsed) && parsed.length >= 2) segs = parsed;
      }
    } catch {}
    setSpin({
      enabled: storeSettings.spin_wheel_enabled !== 'false',
      title: storeSettings.spin_wheel_title || 'UP TO 70% OFF',
      subtitle: storeSettings.spin_wheel_subtitle || 'Try your luck — spin the wheel!',
      segments: segs,
      delay: storeSettings.spin_wheel_delay_seconds || '1',
      autoClose: storeSettings.spin_wheel_auto_close_seconds || '5',
      startAt: storeSettings.spin_wheel_start_at || '',
      endAt: storeSettings.spin_wheel_end_at || '',
    });
    setSpinInit(true);
  }, [storeSettings, spinInit]);

  const handleSaveSpin = async () => {
    try {
      await Promise.all([
        updateSetting.mutateAsync({ key: 'spin_wheel_enabled', value: spin.enabled ? 'true' : 'false' }),
        updateSetting.mutateAsync({ key: 'spin_wheel_title', value: spin.title }),
        updateSetting.mutateAsync({ key: 'spin_wheel_subtitle', value: spin.subtitle }),
        updateSetting.mutateAsync({ key: 'spin_wheel_segments', value: JSON.stringify(spin.segments) }),
        updateSetting.mutateAsync({ key: 'spin_wheel_delay_seconds', value: String(spin.delay) }),
        updateSetting.mutateAsync({ key: 'spin_wheel_auto_close_seconds', value: String(spin.autoClose) }),
        updateSetting.mutateAsync({ key: 'spin_wheel_start_at', value: spin.startAt }),
        updateSetting.mutateAsync({ key: 'spin_wheel_end_at', value: spin.endAt }),
      ]);
  ...
  const updateSeg = (i: number, patch: Partial<SpinSegment>) => {
    setSpin(p => ({ ...p, segments: p.segments.map((s, idx) => idx === i ? { ...s, ...patch } : s) }));
  };
  const addSeg = () => {
    if (spin.segments.length >= 10) return toast.error('Max 10 segments');
    setSpin(p => ({ ...p, segments: [...p.segments, { label: 'NEW', short: 'NEW', color: '#60A5FA', code: '', weight: 5, win: true }] }));
  };
  const removeSeg = (i: number) => {
    if (spin.segments.length <= 2) return toast.error('Min 2 segments');
    setSpin(p => ({ ...p, segments: p.segments.filter((_, idx) => idx !== i) }));
  };
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      ...
      <div className="border border-border p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-light tracking-wide" style={{ fontFamily: 'var(--font-display)' }}>Spin & Win Popup</h3>
            <p className="text-xs text-muted-foreground mt-1">Visitor site e ashle 1s por wheel popup ashbe. Weight diye control korben ke beshi win korbe.</p>
          </div>
          <label className="flex items-center gap-2 text-xs text-muted-foreground">
            <input type="checkbox" checked={spin.enabled} onChange={e => setSpin(p => ({ ...p, enabled: e.target.checked }))} />
            Enabled
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            <div>
              <label className="text-xs text-muted-foreground tracking-wider uppercase block mb-1">Title (big heading)</label>
              <input value={spin.title} onChange={e => setSpin(p => ({ ...p, title: e.target.value }))} className="luxury-input" placeholder="UP TO 70% OFF" />
            </div>
            <div>
              <label className="text-xs text-muted-foreground tracking-wider uppercase block mb-1">Subtitle</label>
              <input value={spin.subtitle} onChange={e => setSpin(p => ({ ...p, subtitle: e.target.value }))} className="luxury-input" placeholder="Try your luck — spin the wheel!" />
            </div>

            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs text-muted-foreground tracking-wider uppercase">Wheel Segments</label>
                <button type="button" onClick={addSeg} className="text-[10px] inline-flex items-center gap-1 border border-border px-2 py-1 hover:bg-muted">
                  <Plus size={12} /> Add
                </button>
              </div>
              <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
                {spin.segments.map((s, i) => (
                  <div key={i} className="border border-border p-2 rounded space-y-1.5 bg-muted/20">
                    <div className="flex items-center gap-2">
                      <input type="color" value={s.color} onChange={e => updateSeg(i, { color: e.target.value })} className="w-8 h-8 border border-border rounded cursor-pointer bg-transparent" />
                      <input value={s.label} onChange={e => updateSeg(i, { label: e.target.value })} className="luxury-input flex-1 text-xs" placeholder="Label (10% OFF)" />
                      <button type="button" onClick={() => removeSeg(i)} className="p-1.5 hover:bg-destructive/10 text-destructive rounded">
                        <Trash2 size={12} />
                      </button>
                    </div>
                    <div className="grid grid-cols-3 gap-1.5">
                      <input value={s.short} onChange={e => updateSeg(i, { short: e.target.value })} className="luxury-input text-xs" placeholder="Short (10%)" title="Wheel e ja dekhabe. New line er jonno \n use korben" />
                      <input value={s.code} onChange={e => updateSeg(i, { code: e.target.value })} className="luxury-input text-xs font-mono" placeholder="CODE" />
                      <input type="number" min={0} value={s.weight} onChange={e => updateSeg(i, { weight: Number(e.target.value) || 0 })} className="luxury-input text-xs" placeholder="Weight" title="Win hobar probability weight. Beshi mane beshi chance." />
                    </div>
                    <label className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                      <input type="checkbox" checked={s.win} onChange={e => updateSeg(i, { win: e.target.checked })} />
                      Win segment (uncheck = Better Luck Next Time)
                    </label>
                  </div>
                ))}
              </div>
          ...
          <div>
            ...
            <div className="bg-muted/30 border border-border p-4 flex items-start justify-center">
              <SpinWheelPopup
                embedded
                titleOverride={spin.title}
                subtitleOverride={spin.subtitle}
                segmentsOverride={spin.segments}
              />
            </div>
          </div>
        </div>
        ...
        <TimeFields
          values={spin}
          onChange={(patch) => setSpin(p => ({ ...p, ...patch }))}
          showDelay
        />

        <div className="flex justify-end">
          <button onClick={handleSaveSpin} disabled={updateSetting.isPending} className="luxury-button-primary inline-flex items-center gap-2 text-[10px]">
            <Save size={14} /> Save Spin Wheel
          </button>
        </div>
      </div>
    </div>
